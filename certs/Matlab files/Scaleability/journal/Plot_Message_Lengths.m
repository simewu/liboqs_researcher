...
if plotNum == 1
    ...
    c = ["#8888FF" "#0000FF" "#000088" "#FF8888" "#FF0000" "#90FF90" "#00FF00" "#008800"];
    
    syms d2(x) d3(x) d5(x) f512(x) f1024(x) shar(x) ssha(x) sshake(x)
    d2(x) = @(x) 2.275e-15*x^2 + 5.045e-06*x + 0.1974;
    d3(x) = @(x) 2.262e-15*x^2 + 5.037e-06*x + 0.2897;
    d5(x) = @(x) 2.217e-15*x^2 + 5.045e-06*x + 0.3375;
...
%         pause;
        p = 0
        if i == 1
            p = fplot(d2, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 2
            p = fplot(d3, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 3
            p = fplot(d5, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 4
            p = fplot(f512, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 5
            p = fplot(f1024, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 6
            p = fplot(shar, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 7
            p = fplot(ssha, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        elseif i == 8
            p = fplot(sshake, '-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        end
        p.ShowPoles = 'off';
        sign_y = data_num(i, 4:4:end-1).';
        plot(x(1:end), sign_y(1:end), '.', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        p=plot([-200, -100], [-200, -100], '.-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
    ...
    set(gca, 'XScale', 'log');
    set(gca, 'YScale', 'log');
    axis([1 100000000 0.03 180000])
    yticks([0.01, 0.1, 1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000])
    legend(appended_lines, appended_titles, 'Location', legend_pos, 'FontSize', 9);
...
elseif plotNum == 2
    ...
    syms d2(x) d5(x) f512(x) f1024(x) shar(x) ssha(x) sshake(x)
    d2(x) = @(x) 2.294e-15*x^2 + 5.043e-06*x + 0.05107;
    d3(x) = @(x) 2.276e-15*x^2 + 5.035e-06*x + 0.08682;
    d5(x) = @(x) 2.17e-15*x^2 + 5.041e-06*x + 0.1377;
...
%         pause;
        ...
        plot(x(1:end), verify_y(1:end), '.', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        p=plot([-200, -100], [-200, -100], '.-', 'LineWidth', 2, 'MarkerSize', 20, 'Color', c(i));
        appended_lines(end+1) = p;
        appended_titles(end+1) = titles(i);
    end
